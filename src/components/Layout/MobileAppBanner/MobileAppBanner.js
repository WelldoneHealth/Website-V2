    ...
    <div className="w-full max-w-[1500px] my-7 p-2 sm:py-9 sm:px-6 lg:px-0 flex justify-center items-start xl:items-end  lg:flexBetween flex-col  lg:flex-row ">
      <div className="w-full lg:w-[62%] order-2 lg:order-1">
        ...
        <div className="my-6 md:my-10 lg:my-10 space-y-2 ">
         ...
         {fetauresInfo.map((item,index)=>   <p key={index} className="max-ssm:text-sm sm:text-lg font-normal">
            <span className="font-semibold">{item.boldText} </span>{item.text} </p>)}
        </div>
        ...
        <div className=" flex gap-x-3 msm:gap-x-5 mt-2 ">
          <button
            ...
            type="button"
          >
                      <Link href="https://play.google.com/store/apps/details?id=com.welldone.welldone_app&hl=en_IN" passHref target="_blank" className="block w-[150px] sm:w-[180px] lg:h-[60px] "  >

            <img className="w-full h-full" src={GOOGLE_PLAY} /> </Link>
          </button> 
          ...
          >
              <Link href="https://play.google.com/store/apps/details?id=com.welldone.welldone_app&hl=en_IN" passHref target="_blank" className="block w-[150px] sm:w-[180px] lg:h-[60px]">
            <img className="w-full h-full" src={APPLE_STORE} /> </Link>
          </button> 
        </div>
      </div>
