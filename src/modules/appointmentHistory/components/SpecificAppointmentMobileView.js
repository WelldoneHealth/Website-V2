...
{/* <div className="w-full px-3 sm:px-6 py-4 flex  gap-x-2 ssm:gap-x-4  mams:gap-x-6 ">
  ...
  <DataCard image={patientDefault} mainText={`${appointmentData?.patient_first_name ?? ''} ${appointmentData?.patient_middle_name ?? ''} ${appointmentData?.patient_last_name ?? '' }`} subText={appointmentData?.reason?.split('_').join(' ') ?? 'N/A'} textSize="text-sm asm:text-lg msm:text-xl capitalize" />
...
{/* <div className="w-full px-3 sm:px-6 py-4 flex gap-x-2 ssm:gap-x-4  mams:gap-x-6 border-b-[1px] border-[#DADADA]">
  ...
  <DataCard image={smallDoc2} mainText={`${appointmentData?.doctor_suffix ?? ''} ${appointmentData?.doctor_name ?? ''} ${appointmentData?.doctor_middle_name ?? ''} ${appointmentData?.doctor_last_name ?? ''}`} subText={appointmentData?.doctor_specialty ?? 'N/A'} textSize="text-sm asm:text-lg msm:text-xl" />
...
<DataCard image={hospital0} mainText={appointmentData?.clinic_name?.split('-')[0] ?? 'N/A'} subText={appointmentData?.clinic_name?.split('-')[1] ?? 'N/A'}  textSize="text-sm asm:text-base msm:text-lg leading-tight" />
...
<Link href={appointmentData?.prescription_link ?? '/'} passHref target='_blank' className='block w-[95%]  msm:w-[200px] ' ><button type="button" className="w-full px-5 py-2 border-[1px] border-primary rounded-[5px]">
    <p className='text-primary font-medium  '>Prescription {!appointmentData?.prescription_link && "-N/A"}</p>
</button> </Link>

<button onClick={()=>router.push(`/doctor-details/${appointmentData?.doctor_slug}/${appointmentData?.branch_slug}`)} type="button" className="w-[95%]  msm:w-[200px] px-5 py-2 border-[1px] border-primary rounded-[5px]">
...
<FollowUpDateComp followUpDate={appointmentData?.followup_date}  />
  {appointmentData?.is_clinic_booking &&  <div className="w-full max-lsm:hidden"> <Msg /></div> }
...
const DataCard=({image,mainText,subText,textSize})=>(
    ...
    <div className="space-y-1 flex flex-col justify-center ">
      ...
      <p className='text-xs asm:text-sm msm:text-base leading-tight capitalize '>{subText}</p>
    </div>
    </div> 
)