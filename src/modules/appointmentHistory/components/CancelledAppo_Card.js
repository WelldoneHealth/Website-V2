...
const [isSmallScreen, setIsSmallScreen] = useState(false);
  ...
  return (
    <div style={{boxShadow:" 1px 1px 4px 2px #8383834D"}}
    onClick={() => isSmallScreen && router.push(`/doctor-details/${appointmentCardData?.doctor_slug}/${appointmentCardData?.branch_slug}`)}
  ...
  {/* ---------2nd part-------------- */}
      ...
      <div className="  space-y-1  lsm:text-nowrap   md:space-y-2">
                  ...
                  {appointmentCardData?.doctor_specialty ?? 'N/A'}</p>
                  <p className="text-xs asm:text-sm msm:text-base md:text-sm">
                  {appointmentCardData?.clinic_name ?? 'N/A'}                   </p>
  ...
  <button type="button" onClick={()=>router.push(`/doctor-details/${appointmentCardData?.doctor_slug}/${appointmentCardData?.branch_slug}`)} className='px-5 py-[6px] w-[250px] justify-center flex items-center gap-x-2 text-sm font-medium border-[1px] border-[#01549A]  rounded-[20px] text-white bg-primary  '>  Book Again </button>
  </div>
      </div>
