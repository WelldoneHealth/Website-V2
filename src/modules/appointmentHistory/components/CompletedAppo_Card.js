...
export default function CompletedAppo_Cards({appointmentCardData,status,isSpecific=false,specificData1=[],specificData2=[]}) {
...
const appo_data2=[
  ...
  return (
    <div
    onClick={() => isSmallScreen && router.push(`/appointment-history/${appointmentCardData?.id}`)}
    ...
    >
      ...
      {!isSpecific &&    (<div className="w-full max-lsm:hidden max-w-[800px]  capitalize max-sm:hidden flex  items-start flex-wrap  gap-x-11 gap-y-3  justify-between text-base lg:text-[13px] esm:text-sm ">
        {/* -------writing 1 st part------- */}
        ...
        {/* ---------------writing 3 rd part----------- */}
       <button onClick={()=>router.push(`/appointment-history/${appointmentCardData?.id}`)}  className="mt-1 text-[#01549A] font-medium px-2 md:px-4 py-2  rounded-[10px] border-[1px] border-[#01549A]  text-sm lg:text-xs esm:text-sm flex justify-center items-center text-nowrap ">
          {" "}
          <span className="block lg:hidden esm:block">+ Show Appointment details</span>      
          <span className="hidden lg:block esm:hidden">Appointment Details</span>
              </button> 
      </div>) }
...
{isSpecific &&    (<div className="w-full max-md:hidden max-w-[800px] px-2  pb-3 capitalize  border-b-[2px]  border-[#DADADA] max-sm:hidden flex   gap-x-24 gap-y-3 text-sm ">
        {/* -------writing 1 st part------- */}
        <div className=" text-nowrap space-y-1">
          
        {specificData1.map((item,index)=><p key={index}>
            <span className="font-semibold">{item.boldText}</span> {item.text}
          </p>)}
        </div>
        {/* ------------writing 2 nd part-------------- */}
        <div className=" text-nowrap space-y-1">
          {specificData2.map((item,index)=><p key={index}>
            <span className="font-semibold">{item.boldText}</span> {item.text}
          </p>)}
            ...
            <div className=" w-full  ">
              <div className="w-full flex   gap-y-3 justify-between  max-md:flex-wrap ">
                <div className="  space-y-1  lsm:text-nowrap   md:space-y-2">
                 ...
                 {appointmentCardData?.doctor_specialty ?? 'N/A'} <span className="text-red-900 text-xs">{appointmentCardData?.doctor_expected_visiting_time}</span>
                  </p>
                  <p className="text-xs asm:text-sm msm:text-base md:text-sm">
                   {appointmentCardData?.clinic_name ?? 'N/A'} 
                  </p>
                  <p className="pt-1 text-xs asm:text-sm msm:text-base md:hidden">
                    Book for Rakesh Nandre ({appointmentCardData?.patient_relation ?? ' - '})
                  </p>
                </div>
              ...
              <div className="mt-4 w-full max-md:hidden  space-y-2">
                <button
                  type="button" 
                  onClick={()=>router.push(`/doctor-details/${appointmentCardData?.doctor_slug}/${appointmentCardData?.branch_slug}`)}
...
{ appointmentCardData?.followup_date ? (remainingDays !== 0 ?
                 ` Follow before – ${ moment(appointmentCardData?.followup_date).format("DD/MM/YYYY") ?? 'N?A'}, ${remainingDays} days remaining​  ` : 
              "FollowUp Today") : "FollowUp Date - N/A " } 
                </p>

                {/* --------------message---------------- */}
            {appointmentCardData?.is_clinic_booking &&  <div className="w-full max-lsm:hidden"> <Msg /></div> }
...
<img src={cardArrow?.src} className="block h-5 sm:h-10  cursor-pointer" alt="load..."  />
      ...
      {appointmentCardData?.is_clinic_booking &&  <div className="w-full pt-2  max-md:hidden block lsm:hidden"> <Msg /></div> }
      </div>
  );
}
