...
export default function page() {
  ...
  const FeaturesComponent =({icon,iconHeading,heading,mainImage,textData})=>(
    <div style={{boxSshadow: "0px 0px 4px 0px #01549A"}} className="w-full px-4 sm:px-6  lg:px-10 py-4 sm:py-6 border-[1px] border-[#DADADA] rounded-3xl flex items-start gap-x-4 lsm:gap-x-8">
<div className="space-y-3 asm:space-y-5">
  <div className="px-3 lsm:px-5 py-2 asm:py-3 bg-secondary rounded-[10px] text-primary text-xs ssm:text-sm asm:text-base lg:text-lg font-medium flex items-center gap-x-3">
    <img src={icon?.src} className=' w-[30px] asm:w-[40px]  lsm:w-auto' alt="load..."  /> <p className='font-medium'>{iconHeading}</p>
  </div>

  <img src={mainImage?.src} className='block mx-auto md:hidden w-[70%] ' alt="load..."  />


  <p className='text-xl asm:text-[22px] lg:text-[28px] font-bold leading-[30px] max-asm:leading-tight'>{heading}</p>
  <div className="space-y-2">
    {textData.map((item)=><div key={item} className="">
      <p className='font-medium text-base'>{item.primaryText}</p>
      <p className='text-xs asm:text-sm'>{item.subText}</p>
    </div>)}
  </div>
</div>
<img src={mainImage?.src} className='max-md:hidden w-[45%] h-auto xl:w-auto' alt="load..."  />
</div>
  )
...
{/* ------------------------main header--------------- */}
<div style={{ background: 'linear-gradient(270deg, rgba(150, 207, 255, 0.5) 36%, rgba(150, 207, 255, 0.1) 92%)' }}
...
<div  className="space-y-12 msm:space-y-14 max-md:order-2  pt-2 md:pt-4  lsm:pt-10  max-lsm:pb-8   relative z-20 "> 
 ...
 {["Continues patients flow","Appointments","Consultation"].map((item,index)=>  <p key={index} className='font-medium text-lg asm:text-[22px] msm:text-[29px] sm:text-[32px] md:text-[24px] lg:text-[31px] xl:text-[35px] 2xl:text-[40px]   ' > Real time <span className='text-primary underline underline-offset-4 ' >{item}</span></p> )}
</div>

<button type="button"  className='mt-6 px-7 text-sm asm:text-base msm:text-lg xl:text-xl  py-2 lsm:py-3 rounded-[30px]  font-semibold  bg-primary text-white '  >Start getting patients</button>
</div>
...
<div className='w-full sm:pt-10 lg:pt-24 xl:pt-40 ' ><Banner2 /></div>
